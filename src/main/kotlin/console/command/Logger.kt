...
object Logger : TreeCommand(Level, Filter, ShowLoggerName)
{
    ...
    object Level : Command
    {
        ...
        override suspend fun execute(sender: CommandSender, args: List<String>): Boolean
        {
            if (args.isEmpty())
            {
                sender.out("logger level: ${ShadowedLogger.globalLogger.logger.level.name}")
            }
            else try
            {
                val level=java.util.logging.Level.parse(args[0])
                ShadowedLogger.setLevel(level)
    ...
    object Filter : TreeCommand(Add, Remove, List, Mode)
    {
        ...
        object Add : Command
        {
            ...
            ): Boolean
            {
                ...
                ShadowedLogger.addFilter(args[0])
        ...
        object Remove : Command
        {
            ...
            ): Boolean
            {
                ...
                ShadowedLogger.removeFilter(args[0])
            ...
            override suspend fun tabComplete(args: kotlin.collections.List<String>): kotlin.collections.List<Candidate>
            {
                if (args.size==1)
                {
                    return ShadowedLogger.filters().map { Candidate(it) }
                }
                return listOf()
            }
        }
        ...
        object List : Command
        {
            ...
            {
                sender.out("Filters:")
                for (filter in ShadowedLogger.filters())
                {
                    sender.out("- $filter")
                }
                return true
            ...
            ): Boolean
            {
                ...
                else
                {
                    when (args[0])
                    {
                        "whitelist" ->
                        {
                            ShadowedLogger.setWhiteList(true)
                            sender.out("set filter mode to whitelist")
                        }
                        "blacklist" ->
                        {
                            ShadowedLogger.setWhiteList(false)
