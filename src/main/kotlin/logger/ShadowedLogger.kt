package moe.tachyon.shadowed.logger
...
import moe.tachyon.shadowed.config.loggerConfig
import moe.tachyon.shadowed.console.AnsiStyle.Companion.RESET
import moe.tachyon.shadowed.console.Console
...
import moe.tachyon.shadowed.logger.ShadowedLogger.getLevelName
import moe.tachyon.shadowed.logger.ShadowedLogger.parseLoggerName
import moe.tachyon.shadowed.logger.ShadowedLogger.safe
import moe.tachyon.shadowed.utils.LineOutputStream
import moe.tachyon.shadowed.utils.LinePrintStream
...
object ShadowedLogger
{
    ...
    fun getLogger(): LoggerUtil = getCallerClass()?.let(ShadowedLogger::getLogger) ?: globalLogger
    ...
    fun String.parseLoggerName(): String =
        if (this.startsWith("moe.tachyon.shadowed.")) "~." + this.substring("moe.tachyon.shadowed.".length)
        else this
}
        ...
        this.formatter = object: Formatter()
        {
            override fun format(record: LogRecord): String
            {
                ...
                val head = if (loggerConfig.showLoggerName) String.format(
                    "%s[%s]%s[%s]%s[%s]%s",
                    PURPLE.bright(),
                    ShadowedLogger.loggerDateFormat.format(record.millis),
                    CYAN.bright(),
...
object ToFileHandler: Handler()
{
    ...
    fun clearOld(duration: Duration = loggerConfig.logFileSaveTime)
    {
        ...
        files.asSequence()
            ...
            .mapNotNull { runCatching { it.first to fileDateFormat.parse(it.second) }.getOrNull() }
